...
export function computeUpdatedCalibration(calibration, analysis, startedAt) {
  ...
  // to an S/M/L bucket requires joining on task IDs in the log entry's context field
  // against the taskAssignments from the quote. That join is future work; until then,
  // bucket defaults in calibration.json are the source for doer_tokens estimates.
