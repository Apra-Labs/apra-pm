...
What it installs:
  ...
  ~/.apra-pm/cost.js          pure JS cost functions extracted from auto-sprint.js (all providers)
...
function main() {
  ...
  //    and write it as a self-contained CommonJS module to ~/.apra-pm/cost.js.
  //    This is the single source of truth for cost functions; cost.md requires it
  //    directly without any vm/slice ceremony. The source file is never copied whole
  //    (it contains Claude Code APIs that don't exist on other providers).
  ...
  if (fs.existsSync(workflowSrc)) {
    ...
    if (blockStart < 0 || blockEnd < blockStart) {
      ...
      const costJs = [
        ...
        '// Re-run: node install.mjs [--llm <provider>] --force',
        '',
        block,
        '',
      ...
      const costDest = path.join(HOME, '.apra-pm', 'cost.js');
      ensureDir(path.dirname(costDest));
